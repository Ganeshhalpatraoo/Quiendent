...
def category(request):
    national = Product.objects.filter(category='national').all()
    englishpremierleague = Product.objects.filter(category='english premier league').all()
    laliga = Product.objects.filter(category='la liga').all()
    bundesliga = Product.objects.filter(category='bundesliga').all()
    seriaA = Product.objects.filter(category='seria A').all()
    ligue1 = Product.objects.filter(category='ligue 1').all()
    premieraliga = Product.objects.filter(category='premiera liga').all()
    isl = Product.objects.filter(category='indian super league').all()

    order = cart(request)
    context = {'national':national, 'englishpremierleague':englishpremierleague, 'laliga': laliga , 'bundesliga':bundesliga,'seriaA':seriaA,'ligue1':ligue1, 'premieraliga':premieraliga,'isl':isl, 'order':order}
